...
public void OnPluginStart()
{
	...
	sb_fix_help_shove_type			= CreateConVar("sb_fix_help_shove_type", "3", "是否允许生还者机器人使用推动感染者进行帮助 <0: 不用推动帮助, 1: 只推Smoker进行帮助, 2: 推Smoker和Jockey进行帮助, 3: 推Smoker、Jockey和Hunter进行帮助 | def: 2>", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	...
	sb_fix_ci_melee_range			= CreateConVar("sb_fix_ci_melee_range", "150", "如果启用了’sb_fix_ci_melee_allow‘ 则生还者机器人使用近战武器清理普通感染者的范围是多少", FCVAR_NOTIFY, true, 1.0, true, 500.0);
	// ---------------------------------
	...
	sb_fix_bash_hunter_range		= CreateConVar("sb_fix_bash_hunter_range", "100", "生还者机器人射击、搜索飞行中的Hunter范围", FCVAR_NOTIFY, true, 1.0, true, 500.0);
	...
	sb_fix_bash_jockey_range		= CreateConVar("sb_fix_bash_jockey_range", "100", "生还者机器人射击、搜索飞行中的Jockey范围", FCVAR_NOTIFY, true, 1.0, true, 500.0);
	// ---------------------------------

...
#define PLUGIN_VERSION	   "1.1.0 2026-05-15"
#define PLUGIN_AUTHOR	   "gvazdas & JBcat, modified by HEIMAO"
...
bool CollectCells(ArrayList allCells, float maxFlow)
{
	...
	for (int i = 0; i < allAreas.Length; i++)
	{
		...
		if (!(L4D_GetNavArea_SpawnAttributes(nav) & NAV_SPAWN_ESCAPE_ROUTE))
			continue;
		if (L4D_NavArea_IsBlocked(nav, TEAM_SURVIVOR, true) || L4D_NavArea_IsBlocked(nav, TEAM_SURVIVOR, false))
			continue;

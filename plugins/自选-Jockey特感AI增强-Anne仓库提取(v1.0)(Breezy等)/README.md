...
源码标注作者：`Breezy、High Cookie、Standalone、Newteee、cravenge、Harry、Sorallll、PaimonQwQ、夜羽真白、东`；Anne 上游插件版本：`2022/11/1`；本商店适配版：`1.0`。
...
该基线已经包含 2026-09-11 合并的 Jockey 空中移动高度判断修复。本条目保留 Anne 仓库中的 AI 逻辑和原作者信息。商店适配将插件版本统一为 `1.0`，增加了 `AutoExecConfig(true, "ai_jockey_new")` 和标准自动配置文件，并将插件自建 ConVar 放入 `anne_ai_jockey_*` 命名空间；最后使用 SourceMod `1.11.0.6968` 重新编译。
...
- `anne_ai_jockey_bhop_speed "80.0"`：每次连跳施加的推进速度。
- `anne_ai_jockey_start_hop_distance "800.0"`：Jockey 距离目标多近时开始主动连跳。
- `anne_ai_jockey_stumble_radius "50.0"`：骑中后令附近生还者硬直的半径；设为 `0` 可关闭额外硬直。
- `anne_ai_jockey_air_angles "60.0"`：当前速度方向与目标方向的夹角超过该值时尝试空中转向，范围为 0～180。
...
- 从旧商店包升级时需要替换 `cfg/sourcemod/ai_jockey_new.cfg`；旧的 `ai_Jockey*` 配置名不再生效。
- 默认 `anne_ai_jockey_stumble_radius "50.0"` 不只是 AI 优化，还会增强骑中后的范围压制。只需要智能移动时建议设为 `0`。

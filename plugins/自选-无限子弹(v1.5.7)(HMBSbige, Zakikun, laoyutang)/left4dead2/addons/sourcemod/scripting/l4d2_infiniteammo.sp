...
public Plugin:myinfo =
{
	...
	author = "HMBSbige && Zakikun && laoyutang",
	description = "Make Players Ammo Infinite",
...
public OnPluginStart()
{
	...
	CreateConVar("l4d2_iammo_version", PLUGIN_VERSION, "L4D2 无限子弹 版本", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	IAmmo = CreateConVar("l4d2_iammo_enable", "2", "是否开启无限子弹? 0=关闭 1=开启 2=所有人", FCVAR_NONE, true, 0.0, true, 2.0);
	AllowNm = CreateConVar("l4d2_iammo_normal", "0", "是否开启普通枪械无限子弹? 0=关闭 1=开启 2=只开启改功能关闭其余", FCVAR_NONE, true, 0.0, true, 2.0);
	AllowGL = CreateConVar("l4d2_iammo_gl", "0", "是否开启榴弹发射器无限子弹? 0=关闭 1=开启 2=只开启改功能关闭其余", FCVAR_NONE, true, 0.0, true, 2.0);
	AllowM60 = CreateConVar("l4d2_iammo_m60", "0", "是否开启M60无限子弹? 0=关闭 1=开启 2=只开启改功能关闭其余", FCVAR_NONE, true, 0.0, true, 2.0);
	AllowChainsaw = CreateConVar("l4d2_iammo_chainsaw", "1", "是否开启电锯无限子弹? 0=关闭 1=开启 2=只开启改功能关闭其余", FCVAR_NONE, true, 0.0, true, 2.0);
	AllowThrowables = CreateConVar("l4d2_iammo_throwables", "0", "是否开启投掷物无限子弹? 0=关闭 1=开启 2=只开启改功能关闭其余", FCVAR_NONE, true, 0.0, true, 2.0);
	AllowUpgradeAmmo = CreateConVar("l4d2_iammo_upgradeammo", "0", "是否开启高爆弹或燃烧弹无限子弹? 0=关闭 1=开启", FCVAR_NONE, true, 0.0, true, 1.0);

	AllowMeds = CreateConVar("l4d2_iammo_meds", "0", "是否开启无限急救包? 0=关闭 1=开启 2=只开启改功能关闭其余", FCVAR_NONE, true, 0.0, true, 2.0);
	AllowDefibs = CreateConVar("l4d2_iammo_defibs", "0", "是否开启无限电击器? 0=关闭 1=开启 2=只开启改功能关闭其余", FCVAR_NONE, true, 0.0, true, 2.0);
	AllowPills = CreateConVar("l4d2_iammo_pills", "0", "是否开启无限止痛药? 0=关闭 1=开启 2=只开启改功能关闭其余", FCVAR_NONE, true, 0.0, true, 2.0);
	AllowShots = CreateConVar("l4d2_iammo_shots", "0", "是否开启无限肾上腺素? 0=关闭 1=开启 2=只开启改功能关闭其余", FCVAR_NONE, true, 0.0, true, 2.0);

	AdminOverride = CreateConVar("l4d2_admin_override", "0", "管理员总是开启以上所有无限子弹? 0=关闭 1=开启", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	AutoExecConfig(true, "l4d2_infiniteammo"); // 自动生成并加载 cfg/sourcemod/l4d2_infiniteammo.cfg。
}
	...
	else if (action == MenuAction_Select)
	{
		if (target > 0)
		{
			if (target == client)
			{
				...
				PrintToChat(client,"\x04[iammo] \x03无限子弹 \x05启用");	
			}
			else if (target == 477)
			{
				new count = 0;
				for (new i=1; i<=MaxClients; i++)
				{
					...
					{
						InfiniteAmmo[i] = 1;
						PrintToChat(client,"\x04[iammo] \x03无限子弹 \x05开启 %N", i);
						PrintToChat(i, "\x04[iammo] \x03你获得了 \x05无限子弹");
						count++;
					}
				}
				if (count == 0)
				{
					PrintToChat(client,"\x04[iammo] \x03未找到玩家或所有玩家都已获得无限子弹");
				}
			}
			else if (target > 0)
			{
				if (IsClientInGame(target) && GetClientTeam(target) == 2 && !IsFakeClient(target) && InfiniteAmmo[target] == 0)
				{
					...
					PrintToChat(client,"\x04[iammo] \x03无限子弹 \x05开启 %N", target);
					PrintToChat(target, "\x04[iammo] \x03你获得了 \x05无限子弹");
				}
			}
	...
	else if (action == MenuAction_Select)
	{
		if (target > 0)
		{
			if (target == client)
			{
				...
				PrintToChat(client,"\x04[iammo] \x03无限子弹 \x05禁用");	
			}
			else if (target == 477)
			{
				new count = 0;
				for (new i=1; i<=MaxClients; i++)
				{
					...
					{
						InfiniteAmmo[i] = 0;
						PrintToChat(client,"\x04[iammo] \x03无限子弹 \x05禁用 %N", i);
						PrintToChat(i,"\x04[iammo] \x03你失去了 \x05无限子弹");
						count++;
					}
				}
				if (count == 0)
				{
					PrintToChat(client,"\x04[iammo] \x03未找到玩家或所有玩家都未获得无限子弹");
				}
			}
			else if (target > 0)
			{
				if (IsClientInGame(target) && GetClientTeam(target) == 2 && !IsFakeClient(target) && InfiniteAmmo[target] == 1)
				{
					...
					PrintToChat(client,"\x04[iammo] \x03无限子弹 \x05禁用 %N", target);
					PrintToChat(target,"\x04[iammo] \x03你失去了 \x05无限子弹");
				}
			}
		...
		else
		{
			for (new i=1; i<=MaxClients; i++)
			{
				if (IsClientConnected(i))
				{
					...
					{
						InfiniteAmmo[i] = 0;
						PrintToChat(i, "\x04[iammo] \x03 你失去了 \x05无限子弹");
					}
					else if (newval == 2) 
					{
						InfiniteAmmo[i] = 1;
						PrintToChat(i, "\x04[iammo] \x03 你获得了 \x05无限子弹");
					}
				}
			}
		}
...
public Action:Command_IAmmo(client, args)
{
	...
	if (EnableVar == 0)
	{
		ReplyToCommand(client, "[iammo] 03 无限子弹未开启");
	...
	if (args < 1)
	{
		...
		{
			ReplyToCommand(client, "[iammo] 你必须在游戏中才能切换无限子弹");	
		}
	}		
	else if (args == 1)
	{
		ReplyToCommand(client, "[iammo] 用法: l4d2_iammo <#userid|name> <0|1>");
	}
	else if (args == 2)
	{
		...
		for (new i=0; i<target_count; i++)
		{
			...
			if (args2 == 0)
			{
				...
				PrintToChat(target_list[i],"\x04[iammo] \x03你失去了 \x05无限子弹");
			}
			else if (args2 == 1)
			{
				ReplyToCommand(client,"[SM] 无限子弹 启用 %s",clientname);	
				InfiniteAmmo[target_list[i]] = 1;
				PrintToChat(target_list[i],"\x04[iammo] \x03你获得了 \x05无限子弹");
			}			
			else
			{
